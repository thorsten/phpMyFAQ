...
class RegistrationHelperTest extends TestCase
{
    ...
    public function testCreateUserWithCollidingIdentityReturnsUniformSuccess(
        ...
        $database
            ->method('query')
            ->willReturnCallback(function (string $sql) use (&$lastQuery): bool {
                $lastQuery = $sql;
                return true;
            });
        $database
            ->method('numRows')
            ->willReturnCallback(function () use (&$lastQuery, $existingEmail, $existingLogin): int {
                if ($existingEmail !== '' && str_contains($lastQuery, "email = '" . $existingEmail . "'")) {
                    return 1;
                }
                if ($existingLogin !== '' && str_contains($lastQuery, "login = '" . $existingLogin . "'")) {
                    return 1;
                }
                return 0;
            });
