...
class MailgunProviderTest extends TestCase
{
    ...
    {
        $this->assertSame(10, MailgunProvider::HTTP_CLIENT_OPTIONS['timeout']);
        $this->assertSame(0, MailgunProvider::HTTP_CLIENT_OPTIONS['max_redirects']);

        // The default client must be constructible from the options (no HTTP call is made)
        $provider = new MailgunProvider($this->createStub(Configuration::class));
