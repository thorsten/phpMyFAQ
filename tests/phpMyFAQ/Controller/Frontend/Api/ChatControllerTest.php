...
final class ChatControllerTest extends ApiControllerTestCase
{
    ...
    public function testSendReturnsUnauthorizedWhenCsrfTokenIsInvalid(): void
    {
        $controller = new ChatController();
        $session = $this->createSession();
        $this->createValidSessionToken($session, 'send-chat-message');
        $this->injectControllerState($controller, $this->createAuthenticatedUserMock(), $session);
    ...
    public function testSendReturnsBadRequestForEmptyMessage(): void
    {
        $controller = new ChatController();
        $session = $this->createSession();
        $csrfToken = $this->createValidSessionToken($session, 'send-chat-message');
        ...
        self::assertSame(Response::HTTP_BAD_REQUEST, $response->getStatusCode());
        self::assertSame('Invalid recipient or empty message', $payload['error']);
    }
    ...
    public function testMarkAsReadReturnsUnauthorizedWhenCsrfTokenIsInvalid(): void
    {
        $this->insertChatMessage(2, 1, 'Unread message', 0, '2026-03-02 10:00:00');

        $controller = new ChatController();
        $session = $this->createSession();
        $this->createValidSessionToken($session, 'mark-chat-read');
        $this->injectControllerState($controller, $this->createAuthenticatedUserMock(), $session);
    ...
    public function testMarkAsReadReturnsBadRequestForInvalidMessageId(): void
    {
        $controller = new ChatController();
        $session = $this->createSession();
        $csrfToken = $this->createValidSessionToken($session, 'mark-chat-read');
        ...
        $request = Request::create('/chat/read/invalid', 'POST', content: json_encode([
            'csrfToken' => $csrfToken,
        ], JSON_THROW_ON_ERROR));
        $request->attributes->set('messageId', 'invalid');

        $response = $controller->markAsRead($request);
        $payload = json_decode((string) $response->getContent(), true, 512, JSON_THROW_ON_ERROR);

        self::assertSame(Response::HTTP_BAD_REQUEST, $response->getStatusCode());
        self::assertSame('Invalid message ID', $payload['error']);
    }
    ...
    public function testGetMessagesUsesDefaultPaginationForInvalidLimitAndOffset(): void
    {
        $this->insertChatMessage(2, 1, 'Hello from partner', 0, '2026-03-02 10:00:00');
        $this->insertChatMessage(1, 2, 'Reply from user', 1, '2026-03-02 10:05:00');
        ...
        $request = Request::create('/chat/messages/2?limit=invalid&offset=invalid', 'GET');
        $request->attributes->set('userId', 2);
    ...
    public function testGetConversationsReturnsConversationSummaryWhenMessagesExist(): void
    {
        $this->upsertUserData(2, 'Beta User', 'beta@example.com');
        $this->insertChatMessage(2, 1, 'Unread message', 0, '2026-03-02 10:00:00');
        ...
        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertTrue($payload['success']);
        self::assertNotEmpty($payload['conversations']);
    }
}
