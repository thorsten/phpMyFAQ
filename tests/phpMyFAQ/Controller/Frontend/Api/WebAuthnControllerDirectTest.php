...
final class WebAuthnControllerDirectTest extends ApiControllerTestCase
{
    ...
    /**
     * Regression test: the per-login challenge generated by prepareForLogin() is only useful if it
     * is written back to the user record. Without it the replay check in authenticate() has nothing
     * to compare against and a captured assertion can be replayed forever.
     */
    public function testPrepareLoginPersistsTheGeneratedChallenge(): void
    {
        ...
        $controller = new WebAuthnController(new AuthWebAuthn($this->configuration), $user);
        $this->injectControllerState($controller, $this->createAuthenticatedUserMock(), $this->createSession());
        ...
        self::assertIsString($persistedKeys, 'The keys carrying the challenge must be persisted.');

        $decoded = json_decode($persistedKeys, false, 512, JSON_THROW_ON_ERROR);
        self::assertIsArray($decoded);
        self::assertNotEmpty($decoded[0]->challenge ?? '', 'The pending challenge must be persisted.');
    }

    /**
     * Regression test: authenticate() blanks the consumed challenge on the keys it is given, and
     * that must be written back so the same assertion cannot be presented twice.
     */
