...
final class StopWordsControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $databasePath = tempnam(sys_get_temp_dir(), 'pmf-admin-stopwords-controller-');
     ...
     * @throws \Exception
     */
    public function testIndexRendersStopWordsManagementPage(): void
    {
        $controller = new StopWordsController();
        $controller->setContainer($this->createControllerContainer());

        $request = Request::create('https://localhost/admin/stopwords');
        $request->attributes->set('_route', 'admin.stopwords');
        $response = $controller->index($request);

        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertStringContainsString('pmf-stop-words-language-selector', (string) $response->getContent());
        self::assertStringContainsString('pmf-stop-words-add-input', (string) $response->getContent());
        self::assertStringContainsString('pmf-stopwords-content', (string) $response->getContent());
    }
