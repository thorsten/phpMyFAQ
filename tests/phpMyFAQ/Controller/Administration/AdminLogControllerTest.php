...
final class AdminLogControllerTest extends TestCase
{
     ...
     * @throws \Exception
     */
    public function testIndexRendersCurrentPageOfAdminLogEntries(): void
    {
        $adminLog = $this->createMock(AdminLog::class);
        $adminLog->method('getNumberOfEntries')->willReturn(16);
        $adminLog->method('getAll')->willReturn($this->buildLoggingData());

        $controller = new AdminLogController();
        $controller->setContainer($this->createControllerContainer($adminLog));

        $request = Request::create('https://localhost/admin/statistics/admin-log?page=2');
        $request->attributes->set('_route', 'admin.statistics.admin-log');
        $response = $controller->index($request);

        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertStringContainsString('id="pmf-export-admin-log"', (string) $response->getContent());
        self::assertStringContainsString('192.0.2.16', (string) $response->getContent());
        self::assertStringNotContainsString('Entry 15', (string) $response->getContent());
    ...
    /**
     * @return list<array{id:int,time:string,userId:int,ip:string,text:string}>
     */
    private function buildLoggingData(): array
    {
        $items = [];
        for ($id = 1; $id <= 16; $id++) {
            $items[] = [
