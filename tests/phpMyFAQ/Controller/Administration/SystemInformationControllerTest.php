...
final class SystemInformationControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $databasePath = tempnam(sys_get_temp_dir(), 'pmf-admin-system-information-controller-');
        ...
        $this->configuration->set('storage.useRedisForConfiguration', false);
    }
     ...
     * @throws \Exception
     */
    public function testIndexRendersSystemInformationWithDisabledSearchBackends(): void
    {
        $controller = new SystemInformationController(new \phpMyFAQ\System());
        $controller->setContainer($this->createControllerContainer());

        $request = Request::create('https://localhost/admin/system');
        $request->server->set('SCRIPT_FILENAME', '/var/www/html/admin/index.php');
        $request->server->set('SERVER_SOFTWARE', 'PHPUnit Web Server');
        $request->server->set('DOCUMENT_ROOT', '/var/www/html');
