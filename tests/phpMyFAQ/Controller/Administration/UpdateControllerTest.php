...
final class UpdateControllerTest extends TestCase
{
     ...
     * @throws \Exception
     */
    public function testIndexRendersNightlyUpdatePage(): void
    {
        $this->configuration->set('upgrade.releaseEnvironment', 'nightly');
        $this->configuration->set('upgrade.dateLastChecked', '2026-03-08 09:00:00');
        $this->configuration->set('main.currentVersion', '4.2.0-alpha');

        $controller = new UpdateController();
        $controller->setContainer($this->createControllerContainer());

        $request = Request::create('https://localhost/admin/update');
        $request->attributes->set('_route', 'admin.update');
        $response = $controller->index($request);

        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertStringContainsString('pmf-button-check-updates', (string) $response->getContent());
        self::assertStringContainsString('versionCurrent', (string) $response->getContent());
        self::assertStringContainsString('Nightly', (string) $response->getContent());
