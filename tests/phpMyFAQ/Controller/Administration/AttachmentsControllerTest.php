...
final class AttachmentsControllerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $databasePath = tempnam(sys_get_temp_dir(), 'pmf-admin-attachments-controller-');
     ...
     * @throws \Exception
     */
    public function testIndexRendersSecondPageOfAttachments(): void
    {
        $collection = $this->createMock(AttachmentCollection::class);
        $collection->method('getBreadcrumbs')->willReturn($this->buildBreadcrumbs());

        $controller = new AttachmentsController($collection);
        $controller->setContainer($this->createControllerContainer());

        $request = Request::create('https://localhost/admin/attachments?page=2');
        $request->attributes->set('_route', 'admin.attachments');
        $response = $controller->index($request);

        self::assertSame(Response::HTTP_OK, $response->getStatusCode());
        self::assertStringContainsString('attachment_25', (string) $response->getContent());
        self::assertStringNotContainsString('attachment_24', (string) $response->getContent());
