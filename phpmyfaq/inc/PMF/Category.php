...
<<<<<<< HEAD
    ...
    public function swapCategories($category_id_1, $category_id_2)
    {
        ...
        foreach ($tables as $pair) {
            foreach ($pair as $_table => $_field) {
                ...
                $q = sprintf('UPDATE %s SET %s = %d WHERE %s = %d',
    ...
    // updated function
    public function getNumberOfRecordsOfCategory($category_restriction = false, $user_id = 1)
    {
        $numRecordsByCat = [];

        if ( $category_restriction == 'true' && $user_id != 1){
