...
describe('handlePasswordToggle', () => {
  ...
  test('should toggle the password visibility when clicked', () => {
    ...
    const toggle1 = document.getElementById('toggle1') as HTMLElement;
    toggle1.click();

    // Check if the first password input type is 'text'
    const passwordInput1 = document.querySelector('input[data-pmf-toggle="toggle1"]') as HTMLInputElement;
    expect(passwordInput1.getAttribute('type')).toBe('text');

    // Check if the first icon has the 'bi-eye-slash' class
    const icon1 = document.getElementById('toggle1_icon') as HTMLElement;
    expect(icon1.classList.contains('bi-eye-slash')).toBe(true);

    // Trigger a click event on the second toggle
    const toggle2 = document.getElementById('toggle2') as HTMLElement;
    toggle2.click();

    // Check if the second password input type is 'text'
    const passwordInput2 = document.querySelector('input[data-pmf-toggle="toggle2"]') as HTMLInputElement;
    expect(passwordInput2.getAttribute('type')).toBe('text');

    // Check if the second icon has the 'bi-eye-slash' class
    const icon2 = document.getElementById('toggle2_icon') as HTMLElement;
...
describe('handlePasswordStrength', () => {
  ...
  beforeEach(() => {
    ...
    passwordElement = document.querySelector('#faqpassword') as HTMLInputElement;
    strengthElement = document.getElementById('strength') as HTMLElement;
  });
