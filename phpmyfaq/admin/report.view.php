...
<form action="./api/export/report" method="post" accept-charset="utf-8">
    <div
        ...
        <h1 class="h2">
            <i aria-hidden="true" class="bi bi-tasks"></i> <?= Translation::get('ad_menu_reports') ?>
        </h1>
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                if ($user->perm->hasPermission($user->getUserId(), PermissionType::REPORTS->value)) {
                    ...
                    $useLastModified = Filter::filterInput(
                        INPUT_POST,
                        'report_last_modified_person',
                        FILTER_VALIDATE_INT
                    );
                        ...
                        foreach ($report->getReportingData() as $data) {
                            ...
                            if ($useCategory) {
                                if (0 != $data['category_parent']) {
                                    printf(
                                        '<td>%s</td>',
                                        Strings::htmlentities($allCategories[$data['category_parent']]['name'] ?? '')
                                    );
                                } else {
                    ...
                    <input type="hidden" name="report_sub_category" id="report_sub_category"
                           value="<?= $useSubcategory ?>">
                    <input type="hidden" name="report_translations" id="report_translations"
