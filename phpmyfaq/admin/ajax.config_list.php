...
function printInputFieldByType($key, $type)
{
    ...
    switch ($type) {
        ...
        case 'input':
        	...
        	$key == 'socialnetworks.twitterAccessTokenKey') {
        		$value = $_SESSION['access_token']['oauth_token'];      		
        	} else if ($faqconfig->get($key) == '' &&
        	$key == 'socialnetworks.twitterAccessTokenSecret') {
...
?>
    <label class="leftconfig"><?php
    	if ($key == 'socialnetworks.twitterConsumerKey') {
	    	...
	    	if ($faqconfig->get('socialnetworks.twitterAccessTokenKey') == '' ||
        	 	$faqconfig->get('socialnetworks.twitterAccessTokenSecret') == '') {	    		
    	       	print '<a href="../services/twitter/redirect.php"><img src="../services/twitter/twitter-signin.png" alt="Sign in with Twitter"/></a>';
        		print "<br />\n<br />\n";
        	} else {
