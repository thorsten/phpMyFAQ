...
class Upgrade extends AbstractSetup
{
    ...
    public function extractPackage(string $path, callable $progressCallback): bool
    {
        ...
        if ($zipFile) {
            // Reject Zip-Slip: no archive entry may be absolute or escape the
            // extraction directory through ../ traversal.
            for ($i = 0; $i < $zipArchive->numFiles; ++$i) {
                ...
                if (
                    ...
                    $zipArchive->close();
                    throw new Exception(message: 'Download package contains an invalid file path.');
                }
            }
    ...
    public function installPackage(callable $progressCallback): bool
    {
        ...
        if ($failedPaths !== []) {
            throw new Exception(
                message: sprintf('Could not copy %d path(s) into the installation directory: ', count($failedPaths))
